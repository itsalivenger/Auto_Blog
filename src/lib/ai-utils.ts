...
export async function summarizeText(text: string): Promise<string> {
  try {
    ...
    const prompt = `
Summarize the following text concisely while keeping all key ideas and essential information intact.
Avoid losing context, and make it clear and readable.
...
`;
    ...
    throw new Error(`Failed to summarize text with AI: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function improveText(text: string): Promise<string> {
  try {
    ...
    const prompt = `
Improve the following text to make it more exciting, vivid, and eventful—while keeping the facts and core idea unchanged.
...
`;
    ...
    return text_res.replace(/#+/g, '').trim();
...
export async function improveForSEO(text: string): Promise<string> {
  try {
    ...
    const prompt = `
Rewrite the following text to improve SEO, make it more hooking, and enhance storytelling—all while keeping the original facts and meaning intact.

Instructions:
- Start with a captivating introduction (surprising fact, bold statement, or provocative question).
- Use vivid, sensory language.
- Naturally include relevant SEO keywords (no stuffing).
- Add engaging headings and subheadings.
- Use open loops or cliffhangers where appropriate.
- Write conversationally with short, active sentences.
- Highlight reader benefit.
- Keep or slightly expand original length.
